...
> CancelOtp200Response CancelOtp(ctx, id).Execute()
...
func main() {
	...
	resp, r, err := apiClient.CancelOtpAPI.CancelOtp(context.Background(), id).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `CancelOtpAPI.CancelOtp``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `CancelOtp`: CancelOtp200Response
	fmt.Fprintf(os.Stdout, "Response from `CancelOtpAPI.CancelOtp`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiCancelOtpRequest struct via the builder pattern
